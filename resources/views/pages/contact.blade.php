...
@section('content')
    ...
    <section class="content-section grid gap-6 lg:grid-cols-5">
        <div class="info-card lg:col-span-3">
            ...
            @if ($errors->any())
                <div class="mt-5 rounded-xl border border-[color:rgba(0,157,49,0.3)] bg-[color:rgba(0,157,49,0.09)] p-4 text-sm">
            ...
            <form action="{{ route('contact.send') }}" method="POST" class="mt-6 space-y-5">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <input id="full_name" name="full_name" type="text" value="{{ old('full_name') }}" class="form-field @error('full_name') border-[color:rgba(0,157,49,0.86)] @enderror" required>
                        @error('full_name')
                    ...
                    <div>
                        ...
                        <input id="email" name="email" type="email" value="{{ old('email') }}" class="form-field @error('email') border-[color:rgba(0,157,49,0.86)] @enderror" required>
                        @error('email')
                            <p class="mt-2 text-sm text-[var(--accent)]">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input id="subject" name="subject" type="text" value="{{ old('subject') }}" class="form-field @error('subject') border-[color:rgba(0,157,49,0.86)] @enderror" required>
                        @error('subject')
                            <p class="mt-2 text-sm text-[var(--accent)]">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea id="message" name="message" rows="6" class="form-field @error('message') border-[color:rgba(0,157,49,0.86)] @enderror" required>{{ old('message') }}</textarea>
                    @error('message')
                        <p class="mt-2 text-sm text-[var(--accent)]">{{ $message }}</p>
                    @enderror
                </div>
        ...
        <aside class="info-card lg:col-span-2">
            ...
            <div class="mt-8 rounded-xl border border-[color:rgba(0,157,49,0.24)] p-4">
