<div x-cloak x-show="offcanvasOpen" class="fixed inset-0 z-[80]" aria-modal="true" role="dialog">
    <div
        class="absolute inset-0 bg-[color:rgba(15,23,42,0.2)] backdrop-blur-sm"
    ...
    <aside
        ...
        class="mega-panel absolute right-0 top-0 flex h-full w-full max-w-md flex-col border-l border-[color:rgba(15,23,42,0.14)] p-6"
    >
        ...
        <section class="mt-8">
            ...
            <div class="mt-3 space-y-2">
                <template x-for="video in videos.slice(0, 4)" :key="video.title">
                    <a :href="video.url" class="group flex items-center gap-3 rounded-xl border border-[color:rgba(15,23,42,0.14)] p-2 transition duration-200 hover:border-[color:rgba(31,181,111,0.45)]" @click="closeOffcanvas()">
                        <div
                            class="h-12 w-20 flex-none rounded-lg border border-[color:rgba(15,23,42,0.12)] bg-[color:rgba(255,255,255,0.96)] bg-cover bg-center"
                            :style="`background-image:url('${video.thumb}')`"
                        ></div>
                        <div class="min-w-0">
                            <p class="truncate text-sm font-semibold text-[color:rgba(15,23,42,0.92)]" x-text="video.title"></p>
