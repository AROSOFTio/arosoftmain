...
class BlogFeedController extends Controller
{
    ...
    private function buildRssXml(Collection $posts): string
    {
        ...
        return implode("\n", [
            ...
            '<rss version="2.0">',
            '    <channel>',
            '        <title>'.$this->xml('AROSOFT Innovations Ltd.').'</title>',
            '        <link>'.$this->xml((string) route('home')).'</link>',
            '        <description>'.$this->xml('Official website updates from AROSOFT Innovations Ltd.').'</description>',
